...
def display_recommendations(results, budget):
    ...
    with col1:
        ...
        if roi > 0.5:
            st.success(f"Excellent ROI: {roi*100:.1f}%")
        elif roi > 0.15:
            st.info(f"Good ROI: {roi*100:.1f}%")
        elif roi > 0:
            st.warning(f"Low ROI: {roi*100:.1f}%")
        else:
            st.error(f"Negative ROI: {roi*100:.1f}%")
    ...
    with col2:
        ...
        if campaigns and len(campaigns) > 0:
            ...
            for i in range(num_to_show):
                ...
                roi = campaign.get('roi', campaign.get('expected_roi', 0))
                response_rate = campaign.get('response_probability', campaign.get('response_rate', 0))
                target_size = campaign.get('target_size', campaign.get('segment_size', 0))
                
                # Calculate profit
                profit = revenue - cost
                
                # Calculate ROI if not provided or if it's 0
                if roi == 0 and cost > 0:
                    roi = (revenue - cost) / cost
                ...
                if roi > 0:
                    roi_score = min(100, roi * 100)  # Cap at 100
